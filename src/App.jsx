...
function EnemyPortrait({ enemyId, size = 56, style = {} }) {
    const MAP = {
        ...
        "mire_stalker":       { sheetKey: "zone5", col: 0, row: 0, yOffset: 0.02 },
        "rotfang_beast":      { sheetKey: "zone5", col: 1, row: 0, yOffset: 0.00 },
        "plague_channeler":   { sheetKey: "zone5", col: 2, row: 0, yOffset: 0.05 },
        "bog_knight":         { sheetKey: "zone5", col: 0, row: 1, yOffset: 0.08 },
        "swamp_wraith":       { sheetKey: "zone5", col: 1, row: 1, yOffset: 0.10 },
        "bone_legionnaire":   { sheetKey: "zone6", col: 0, row: 0, yOffset: 0.05 },
        "grave_arcanist":     { sheetKey: "zone6", col: 1, row: 0, yOffset: 0.03 },
        "crypt_assassin":     { sheetKey: "zone6", col: 2, row: 0, yOffset: 0.08 },
        "sanctified_fallen":  { sheetKey: "zone6", col: 0, row: 1, yOffset: 0.05 },
        "soul_binder_z6":     { sheetKey: "zone6", col: 1, row: 1, yOffset: 0.08 },
        "void_harbinger":     { sheetKey: "zone7", col: 0, row: 0, yOffset: 0.08 },
        "timebroken_knight":  { sheetKey: "zone7", col: 1, row: 0, yOffset: 0.05 },
        "void_seraph":        { sheetKey: "zone7", col: 2, row: 0, yOffset: 0.02 },
        "chrono_beast":       { sheetKey: "zone7", col: 0, row: 1, yOffset: 0.08 },
        "paradox_shade":      { sheetKey: "zone7", col: 1, row: 1, yOffset: 0.08 },
        "oblivion_knight":    { sheetKey: "zone8", col: 0, row: 0, yOffset: 0.05 },
        "entropy_beast":      { sheetKey: "zone8", col: 1, row: 0, yOffset: 0.02 },
        "soul_binder_z8":     { sheetKey: "zone8", col: 2, row: 0, yOffset: 0.02 },
        "rift_phantom":       { sheetKey: "zone8", col: 0, row: 1, yOffset: 0.08 },
        "soul_binder_z8b":    { sheetKey: "zone8", col: 1, row: 1, yOffset: 0.08 },
...
const CLASSES = {
    "Holy Knight": {
        ...
        abilities: [
            { name: "Divine Strike", cost: 10, desc: "ATK + DEF + SPD · Holy dmg", damage: [18, 28], type: "atk" , scale: "Holy · ATK + DEF + SPD"},
    ...
    "Death Knight": {
        ...
        abilities: [
            { name: "Dark Sacrifice", cost: 0, desc: "5% HP Cost · +50% ATK & DEF x6", damage: [0, 0], type: "darkSacrifice" , scale: "5% HP Cost · Flat buff"},
            { name: "Soul Rend", cost: 15, desc: "ATK + SPD · Ignores 30% DEF", damage: [25, 38], type: "soulRend" , scale: "ATK + SPD · Ignores DEF"},
            { name: "Death's Suffering", cost: 18, desc: "Enemy Max HP · 8% DoT x6", damage: [0, 0], type: "deathSuffering" , scale: "Max HP · DoT"},
        ]
    },
    "Infernal Warden": {
...
function AnimatedBar({ val, max, color, label }) {
...
// Visual overlay that plays on top of a card when an action fires.
// Shows a background sweep + centered emoji + value label all in one place.
const ANIM_CFG = {
...
function CombatAnimOverlay({ anim }) {
    if (!anim) return null;
    const cfg = anim.type ? (ANIM_CFG[anim.type] || ANIM_CFG.slash) : null;
    const keyframe = cfg ? cfg.keyframe : "floatUp";
    const dur = cfg ? cfg.dur : 900;
    return (
        <div style={{ position: "absolute", inset: 0, borderRadius: 10, overflow: "hidden", pointerEvents: "none", zIndex: 10 }}>
            {/* Background sweep — only when a type is set */}
            {cfg && <div style={{ position: "absolute", inset: 0, background: cfg.bg, animation: `${keyframe} ${dur}ms ease-out forwards` }} />}
            {/* Centered emoji + value label together */}
            <div style={{
                position: "absolute", top: "50%", left: "50%",
                transform: "translate(-50%, -50%)",
                display: "flex", alignItems: "center", gap: 5,
                animation: `${keyframe} ${dur}ms ease-out forwards`,
                ...
                {anim.label && (
                    <span style={{
                        ...
                        color: anim.labelColor || "#fff",
                        textShadow: `0 0 8px ${anim.labelColor || "#fff"}99, 0 1px 3px #000`,
                        whiteSpace: "nowrap",
                    }}>{anim.label}</span>
                )}
            </div>
...
const SFX = {
    ...
    dark:      () => { playTone({ freq: 260, freq2: 180, type: "sine", duration: 0.28, volume: 0.12 }); playTone({ freq: 200, freq2: 140, type: "triangle", duration: 0.2, volume: 0.08, delay: 0.12 }); },
...
export default function App() {
    ...
    const startCombat = () => {
        ...
        setSe({ burn: 0, stunned: false, dodgeReady: false, flightBonus: 0, enemyDot: 0, playerPoison: 0, plagueDot: 0, enemyBlind: 0, demonPactBonus: 0, cursedPlateOn: hasP(equipped, "cursedPlate"), frailCurse: 0 });
        setTurn("player"); setTrinketUsed(false);
    ...
    const useTrinket = () => {
        ...
        else if (t.activeType === "mpSurge") { np.mp = clamp(np.mp + 25, 0, np.maxMp); triggerAnim("player", "heal", "+25💧", "#60c0ff"); addLog(`${t.icon} ${t.activeName}! +25 MP!`, "#60c0ff"); }
        else if (t.activeType === "mend") { np.hp = clamp(np.hp + 35, 0, np.maxHp); triggerAnim("player", "heal", "+35❤️", "#ff6090"); addLog(`${t.icon} ${t.activeName}! +35 HP!`, "#ff6090"); }
    ...
    const playerAction = (type, payload) => {
        ...
        const effEnemyDef = ne.def + nb.enemy.filter(b => b.stat === "def" && b.amount > 0).reduce((s, b) => s + b.amount, 0);
        ...
        // tickStartEffects: runs only on committed actions — not on already-active / not-enough-MP no-ops
        const tickStartEffects = () => {
            if (cse.cursedPlateOn) { np.hp = clamp(np.hp - 3, 0, np.maxHp); triggerAnim("player", "dark", "-3💀", "#cc2222"); if (np.hp <= 0) { setPlayer(np); setFinalLog([...log]); playSfx('gameover'); setScreen("gameover"); return true; } }
            ...
            if (cse.pyroclasmDot > 0) { const pyroDmg = Math.max(1, Math.floor(ne.maxHp * 0.08)); ne.hp = Math.max(0, ne.hp - pyroDmg); cse.pyroclasmDot--; triggerAnim("enemy", "fire", `-${pyroDmg}🔥`, "#ff6600"); addLog(`🔥 Pyroclasm burns ${ne.name} for ${pyroDmg}! (${cse.pyroclasmDot} turns left)`, "#ff6600"); if (ne.hp <= 0) { nb.player = nb.player.map(b => ({ ...b, turns: b.turns - 1 })).filter(b => b.turns > 0); setSavedEnemy({...ne}); actionLockRef.current = false; resolveVictory(np, ne, nb, inv, g, eq, cse, rl); return true; } }
            if (cse.burn > 0) { np.hp = clamp(np.hp - 3, 0, np.maxHp); cse.burn--; triggerAnim("player", "fire", "-3🔥", "#ff6030"); if (np.hp <= 0) { setPlayer(np); setFinalLog([...log]); playSfx('gameover'); setScreen("gameover"); return true; } }
            if (cse.playerPoison > 0) { np.hp = clamp(np.hp - 3, 0, np.maxHp); cse.playerPoison--; triggerAnim("player", "poison", "-3🐍", "#80ff80"); if (np.hp <= 0) { setPlayer(np); setFinalLog([...log]); playSfx('gameover'); setScreen("gameover"); return true; } }
            if (cse.plagueDot > 0) { const pd = Math.max(1, Math.floor(np.maxHp * 0.15)); np.hp = clamp(np.hp - pd, 0, np.maxHp); cse.plagueDot--; triggerAnim("player", "poison", `-${pd}☣️`, "#cc44ff"); if (np.hp <= 0) { setPlayer(np); setFinalLog([...log]); playSfx('gameover'); setScreen("gameover"); return true; } }
            if (cse.enemyDot > 0 && ne.hp > 0) { const dd = Math.max(1, Math.floor(ne.maxHp * 0.08)); ne.hp -= dd; cse.enemyDot--; triggerAnim("enemy", "dark", `-${dd}💀`, "#a0ffa0"); if (ne.hp <= 0) { setEnemy(ne); setPlayer(np); actionLockRef.current = false; setTimeout(() => resolveVictory(np, ne, nb, inv, g, eq, cse, rl), 700); return true; } }
            return false;
        };
        ...
        if (type === "attack") {
            ...
            const dmg = calcDmg(c ? Math.floor((raw + fb) * 1.5 * demonMult) : (raw + fb) * demonMult | 0, effEnemyDef);
            dealDmg(dmg, "⚔️ Your attack", c);
            ...
            const ab = payload;
            if (ab.type === "brace") { if (np.mp < ab.cost) { addLog("Not enough MP!", "#ff9060"); actionLockRef.current = false; setPlayer(np); return; } if (cse.braceActive > 0) { addLog("🛡️ Holy Shield already active!", "#f0c060"); actionLockRef.current = false; setPlayer(np); return; } np.mp -= ab.cost; np.mp = clamp(np.mp + regen, 0, np.maxMp); const baseDefB = ep.def + rb.def; const db = Math.max(1, Math.floor(baseDefB * 0.5)); cse.braceActive = 6; nb.player.push({ stat: "def", amount: db, turns: 6, tag: "brace" }); triggerAnim("player", "shield", `🛡️+${db}DEF`, "#f0c060"); addLog(`🛡️ Holy Shield! +${db} DEF for 6 turns!`, "#f0c060"); nb.player = nb.player.map(b => { if (b.tag === "brace" || b.tag === "demonPact" || b.tag === "darkSacrifice" || b.tag === "arcaneBoost") return b; return { ...b, turns: b.turns - 1 }; }).filter(b => b.turns > 0); if (tickStartEffects()) return; commitHolyAura(); setSe(cse); setPlayer(np); setBuffs(nb); actionLockRef.current = false; setTurn("enemy"); setTimeout(() => enemyTurn(np, ne, nb, inv, g, eq, cse, rl), 900); return; }
            if (ab.type === "darkSacrifice") { const hc = Math.max(1, Math.floor(np.hp * 0.05)); if (np.hp - hc <= 0) { addLog("⚠️ Not enough HP!", "#ff6060"); actionLockRef.current = false; setPlayer(np); return; } if (nb.player.some(b => b.tag === "darkSacrifice")) { addLog("💀 Dark Sacrifice already active!", "#cc2222"); actionLockRef.current = false; setPlayer(np); return; } np.hp -= hc; np.mp = clamp(np.mp + regen, 0, np.maxMp); const baseAtkForSac = ep.atk + rb.atk; const baseDefForSac = ep.def + rb.def; const ab2 = Math.floor(baseAtkForSac * 0.5); const db2 = Math.floor(baseDefForSac * 0.5); nb.player.push({ stat: "atk", amount: ab2, turns: 6, tag: "darkSacrifice" }); nb.player.push({ stat: "def", amount: db2, turns: 6, tag: "darkSacrifice" }); triggerAnim("player", "power", `💀+${ab2}ATK`, "#cc2222"); addLog(`💀 Dark Sacrifice! -${hc} HP => ATK+${ab2}, DEF+${db2} x 6!`, "#cc2222"); nb.player = nb.player.map(b => { if (b.tag === "holyShield" || b.tag === "demonPact" || b.tag === "darkSacrifice" || b.tag === "arcaneBoost") return b; return { ...b, turns: b.turns - 1 }; }).filter(b => b.turns > 0); if (tickStartEffects()) return; commitHolyAura(); setSe(cse); setPlayer(np); setBuffs(nb); actionLockRef.current = false; setTurn("enemy"); setTimeout(() => enemyTurn(np, ne, nb, inv, g, eq, cse, rl), 900); return; }
            if (ab.type === "demonPact") { if (cse.demonPactBonus > 0) { addLog("👹 Demon Pact already active!", "#c060f0"); actionLockRef.current = false; setPlayer(np); return; } np.mp -= ab.cost; np.mp = clamp(np.mp + regen, 0, np.maxMp); cse.demonPactBonus = 0.30; nb.player.push({ stat: "atk", amount: 0, turns: 6, tag: "demonPact" }); triggerAnim("player", "dark", "👹+30%DMG", "#c060f0"); addLog(`👹 Demon Pact! +30% dmg x 6 turns!`, "#c060f0"); nb.player = nb.player.map(b => { if (b.tag === "demonPact") return b; return { ...b, turns: b.turns - 1 }; }).filter(b => b.turns > 0); if (tickStartEffects()) return; commitHolyAura(); setSe(cse); setPlayer(np); setBuffs(nb); actionLockRef.current = false; setTurn("enemy"); setTimeout(() => enemyTurn(np, ne, nb, inv, g, eq, cse, rl), 900); return; }
            ...
            if (ab.type === "atk") { const c = isCrit(totalCrit); const atkBonus = Math.floor(totalAtk * 0.4); const defBonus = ab.name === "Divine Strike" ? Math.floor(totalDef * 0.3) : 0; const raw = rand(ab.damage[0], ab.damage[1]) + atkBonus + defBonus; const fb = cse.flightBonus > 0 ? Math.floor(raw * 0.3) : 0; const dmg = calcDmg(c ? Math.floor((raw + fb) * 1.5 * abilBonus * spdMult * demonMult) : Math.floor((raw + fb) * abilBonus * spdMult * demonMult), effEnemyDef); const animMap = { "Divine Strike": "holy", "Hellfire": "fire", "Arcane Bolt": "arcane", "Mana Burst": "arcane", "Snipe": "arrow" }; const animType = animMap[ab.name] || "slash"; dealDmg(dmg, `✨ Your ${ab.name}`, c, animType); }
            else if (ab.type === "soulRend") { const c = isCrit(totalCrit); const atkBonus = Math.floor(totalAtk * 0.4); const raw = rand(ab.damage[0], ab.damage[1]) + atkBonus; const dmg = calcDmg(c ? Math.floor(raw * 1.5 * spdMult * demonMult) : Math.floor(raw * spdMult * demonMult), Math.floor(effEnemyDef * 0.7)); dealDmg(dmg, "💀 Your Soul Rend", c, "dark"); }
            else if (ab.type === "deathSuffering") { if (cse.enemyDot > 0) { addLog("💀 Death's Suffering already active!", "#a0ffa0"); np.mp = clamp(np.mp + ab.cost - regen, 0, np.maxMp); actionLockRef.current = false; setPlayer(np); return; } cse.enemyDot = 6; triggerAnim("enemy", "dark", "💀DoT", "#a0ffa0"); addLog("💀 Your Death's Suffering — 8% HP/turn x6!", "#a0ffa0"); }
            ...
            else if (ab.type === "arcaneBoost") { if (nb.player.some(b => b.tag === "arcaneBoost")) { addLog("🔮 Arcane Surge already active!", "#60c0f0"); np.mp = clamp(np.mp + ab.cost - regen, 0, np.maxMp); actionLockRef.current = false; setPlayer(np); return; } nb.player.push({ stat: "spd", amount: 14, turns: 6, tag: "arcaneBoost" }); nb.player.push({ stat: "manaRegen", amount: 5, turns: 6, tag: "arcaneBoost" }); triggerAnim("player", "arcane", "🔮+14SPD +5MP/t", "#60c0f0"); addLog(`🔮 Arcane Surge! +14 SPD, +5 MP/turn x 6!`, "#60c0f0"); nb.player = nb.player.map(b => { if (b.tag === "holyShield" || b.tag === "demonPact" || b.tag === "darkSacrifice" || b.tag === "arcaneBoost") return b; return { ...b, turns: b.turns - 1 }; }).filter(b => b.turns > 0); if (tickStartEffects()) return; commitHolyAura(); setSe(cse); setPlayer(np); setBuffs(nb); actionLockRef.current = false; setTurn("enemy"); setTimeout(() => enemyTurn(np, ne, nb, inv, g, eq, cse, rl), 900); return; }
            ...
            else if (ab.type === "divineWrath") { const c = isCrit(totalCrit); const base = Math.floor(np.maxHp * 0.20); const dmg = calcDmg(c ? Math.floor(base * 1.5 * spdMult * demonMult) : Math.floor(base * spdMult * demonMult), effEnemyDef); dealDmg(dmg, "😇 Your Divine Wrath", c, "holy"); }
            else if (ab.type === "shadowStep") { if (cse.dodgeReady || cse.flightBonus > 0) { addLog("🌑 Shadow Step already active!", "#60f0a0"); np.mp = clamp(np.mp + ab.cost - regen, 0, np.maxMp); actionLockRef.current = false; setPlayer(np); return; } cse.dodgeReady = true; cse.flightBonus = 2; cse.shadowStepActive = true; triggerAnim("player", "smoke", "🌑 Shadow Step!", "#60f0a0"); addLog("🌑 Your Shadow Step — next attack dodged, +50% Crit for 2 turns!", "#60f0a0"); }
            else if (ab.type === "takeFlight") { if (cse.dodgeReady || cse.flightBonus > 0) { addLog("😇 Take Flight already active!", "#e8e0ff"); np.mp = clamp(np.mp + ab.cost - regen, 0, np.maxMp); actionLockRef.current = false; setPlayer(np); return; } cse.dodgeReady = true; cse.flightBonus = 2; cse.shadowStepActive = false; triggerAnim("player", "flight", "😇 Flight+Dodge", "#e8e0ff"); addLog("😇 Your Take Flight — next attack dodged, +30% dmg x2!", "#e8e0ff"); }
            else if (ab.type === "manaSurge") { const mpBurn = Math.min(30, np.mp); np.mp -= mpBurn; np.mp = clamp(np.mp + regen, 0, np.maxMp); const c = isCrit(totalCrit); const atkBonus2 = Math.floor(totalAtk * 0.4); const raw2 = rand(ab.damage[0], ab.damage[1]) + atkBonus2; const surgeBonus = Math.floor(mpBurn * 1.5); const dmg2 = calcDmg(c ? Math.floor((raw2 + surgeBonus) * 1.5 * abilBonus * spdMult * demonMult) : Math.floor((raw2 + surgeBonus) * abilBonus * spdMult * demonMult), effEnemyDef); dealDmg(dmg2, "💥 Your Mana Surge", c, "arcane"); addLog(`💥 Mana Surge! Burned ${mpBurn} MP for +${surgeBonus} bonus dmg!`, "#44bbff"); }
            else if (ab.type === "timeWarp") { if (cse.timeWarpActive) { addLog("⏰ Time Warp already used this turn!", "#44bbff"); np.mp = clamp(np.mp - regen, 0, np.maxMp); actionLockRef.current = false; setPlayer(np); return; } np.mp -= ab.cost; np.mp = clamp(np.mp + regen, 0, np.maxMp); cse.timeWarpActive = true; triggerAnim("player", "arcane", "⏰ WARP!", "#44bbff"); addLog("⏰ Time Warp! Your next ability or attack is FREE — enemy turn skipped!", "#44bbff"); setSe(cse); setPlayer(np); setBuffs(nb); setEnemy(ne); return; /* Stay on player turn */ }
            else if (ab.type === "apocalypse") {
                const hcAp = Math.floor(np.hp * 0.35);
                if (np.hp - hcAp <= 0) { addLog("⚠️ Not enough HP for Apocalypse!", "#ff6060"); actionLockRef.current = false; setPlayer(np); return; }
                np.hp -= hcAp;
                np.mp = clamp(np.mp + regen, 0, np.maxMp);
                ...
                if (nb.player.some(b => b.tag === "undyingRage")) { addLog("☠️ Undying Rage already active!", "#cc2222"); np.mp = clamp(np.mp + ab.cost - regen, 0, np.maxMp); actionLockRef.current = false; setPlayer(np); return; }
                np.mp -= ab.cost; np.mp = clamp(np.mp + regen, 0, np.maxMp);
                ...
                if (tickStartEffects()) return; commitHolyAura(); setSe(cse); setPlayer(np); setBuffs(nb); actionLockRef.current = false; setTurn("enemy"); setTimeout(() => enemyTurn(np, ne, nb, inv, g, eq, cse, rl), 900); return;
            }
            else if (ab.type === "bloodRitual") {
                const hcBR = Math.floor(np.hp * 0.30);
                if (np.hp - hcBR <= 0) { addLog("⚠️ Not enough HP for Blood Ritual!", "#ff6060"); actionLockRef.current = false; setPlayer(np); return; }
                np.hp -= hcBR;
                np.mp = clamp(np.mp + regen, 0, np.maxMp);
                ...
                const dmg = calcDmg(c ? Math.floor(base * 1.5 * abilBonus * spdMult * demonMult) : Math.floor(base * abilBonus * spdMult * demonMult), effEnemyDef);
            ...
            if (rand(1, 100) > 40) { addLog("🏃 Fled!", "#f0c060"); setCombat(false); setEnemy(null); actionLockRef.current = false; setPlayer(np); return; }
        ...
        if (cse.timeWarpActive) { cse.timeWarpActive = false; if (tickStartEffects()) return; commitHolyAura(); setSe(cse); setPlayer(np); setEnemy(ne); setBuffs(nb); actionLockRef.current = false; return; }
        ...
        nb.player = nb.player.map(b => { if (b.tag === "holyShield" || b.tag === "brace" || b.tag === "darkSacrifice" || b.tag === "arcaneBoost" || b.tag === "demonPact" || b.tag === "undyingRage" || b.tag === "bloodRitual") return b; return { ...b, turns: b.turns - 1 }; }).filter(b => b.turns > 0);
        nb.enemy = nb.enemy.map(b => ({ ...b, turns: b.turns - 1 })).filter(b => b.turns > 0);
        setSe(cse); setSavedEnemy({ ...ne });
        if (ne.hp <= 0) {
            // Show the killing blow animation — lock turn so no further input possible
            setEnemy(ne); setPlayer(np); setBuffs(nb); setTurn("resolving");
            setTimeout(() => { actionLockRef.current = false; resolveVictory(np, ne, nb, inv, g, eq, cse, rl); }, 700);
            return;
        }
        if (tickStartEffects()) return; commitHolyAura(); actionLockRef.current = false;
        ...
        if (fne.style === "duel") {
            ...
            if (fne.id === "infernal_behemoth" && rand(1, 100) <= 30 && fnp.hp > 0) {
                ...
                if (roll2 <= 50) {
                    ...
                    triggerAnim("enemy", "fire", "😤+8ATK", "#ff4400");
                    addLog(`😤 Infernal Behemoth Frenzy Roar! ATK +8 for 2 turns!`, "#ff4400");
                }
            }
        }
        else if (fne.style === "defensive") { if (rand(1, 100) > 50) doHit(eAtk - 2, pDef); else if (!fnb.enemy.some(b => b.tag === "enemyBrace")) { const defGain = Math.max(5, Math.floor(fne.def * 0.35)); fnb.enemy.push({ stat: "def", amount: defGain, turns: 8, tag: "enemyBrace" }); triggerAnim("enemy", "shield", `🛡️+${defGain}DEF`, "#f0a060"); addLog(`${fne.icon || "🛡️"} ${fne.name} braces! +${defGain} DEF for 4 turns`, "#f0a060"); } else doHit(eAtk - 2, pDef); }
            ...
            if (fne.id === "demon_lord_falaxir" && rand(1, 100) <= 30) {
                if (rand(1, 100) <= 50) {
                    ...
                    triggerAnim("player", "dark", "🌑-4DEF", "#cc44ff");
            ...
            // Veltharion the Undying — Soul Leech + Curse of Frailty
                ...
                if (rand(1, 100) <= 50) {
                    ...
                    triggerAnim("enemy", "drain", `+${heal}🩸`, "#cc44ff");
                    ...
                    triggerAnim("player", "dark", "💀Frail", "#aa44ff");
            ...
            // Abyssal Overlord — Mind Shatter + Void Pull
                ...
                if (rand(1, 100) <= 50) {
                    ...
                    triggerAnim("player", "arcane", "👁️Shatter", "#cc00ff");
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                                ...
                                {se.enemyDot > 0 && <StatusPill label={`💀DoT(${se.enemyDot})`} color="#a0ffa0" />}
                                {buffs.enemy.filter(b => b.amount < 0).map((b, i) => {
                                    const rounds = Math.ceil(b.turns / 2);
                                    const icon = b.tag === "smokeBomb" ? "💨" : "⬇️";
                                    return <StatusPill key={`ed${i}`} label={`${icon}${b.stat.toUpperCase()}${b.amount}(${rounds})`} color="#60f0a0" />;
                                })}
                                {buffs.enemy.filter(b => b.amount > 0).map((b, i) => {
                                    const rounds = Math.ceil(b.turns / 2);
                                    const icon = b.tag === "enemyBrace" ? "🛡️" : b.stat === "atk" ? "⚔️" : "⬆️";
                                    return <StatusPill key={`ep${i}`} label={`${icon}${b.stat.toUpperCase()}+${b.amount}(${rounds})`} color="#f0a060" />;
                                })}
                            </div>
            ...
            {!combat && !lvlUp && (
                <div>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 5 }}>
                        ...
                        <Btn onClick={() => { setShowShop(s => { if (!s || shopTab === "sell") { setShopTab("consumables"); return true; } return false; }); setShowEquip(false); playSfx('tab'); }} border="#60c0f0" bg="#0d1a2e" color="#60c0f0">🏪 Shop</Btn>
